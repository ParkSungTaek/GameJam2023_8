...
public class SoundPlayerController : MonoBehaviour
{
    ...
    /// 현재 플레이중인 음악 버튼 타입 
    /// </summary>
    int[] ButtonType = new int[TYPENUM];

    /// <summary>
    /// 플래이 대기 슬라이더
    /// </summary>
    Slider[] SliderBackground { get; set; }

    bool[] _isWaiting = new bool[TYPENUM] { false, false, false, false };
    float[] _isWaitingTime = new float[TYPENUM];

    public void Init(GameObject[] sliderBackground)
    {
        for (int i = 0; i < TYPENUM; i++)
        {
            SliderBackground[i] = sliderBackground[i].GetComponent<Slider>();
        }
    }
    ...
    public void Add(int buttonIdx)
    {
        ...
        if ((ButtonType[audioIdx] != buttonIdx))
        {
            if (SliderBackground[audioIdx].gameObject.activeSelf == false)
            {
                SliderBackground[audioIdx].gameObject.SetActive(true);
                FillSliderOverTime(SliderBackground[audioIdx]);
...
public class SoundPlayerController : UI_Scene
{
    ...
    public override void Init()
    {
        ...
        soundPlayerController.Init(SliderBackground);
    }
